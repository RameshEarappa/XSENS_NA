report 72108 "R407 Purchase - Cr Memo DCR"
{
    ...
    dataset
    {
        dataitem(InvHdr; "Purch. Cr. Memo Hdr.")
        {
            ...
            column(AppliesToText; wgAppliesToText)
            {
            }
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                column(PurchCreditMemoTitle; STRSUBSTNO(wlFncDocumentCaption, wgCopyText))
                {
                }
                ...
                dataitem(InvLine; "Purch. Cr. Memo Line")
                {
                    ...
                    column(UOM; "Unit of Measure")// "Unit of Measure Code")//wgCduDocCreatorTransLationMgt.wgFncGetUnitOfMeaseTrl("Unit of Measure Code")//Krishna)/Krisna
                    {
                    }
                ...
                begin
                    IF NOT CurrReport.PREVIEW THEN
                        CODEUNIT.RUN(CODEUNIT::"PurchCrMemo-Printed", InvHdr);
                end;
            ...
            begin
                ...
                wlFncGetPurchCreditMemoLines(InvHdr, InvLine);
    ...
    requestpage
    {
        ...
        begin
            ...
            wgLogInteraction := wgCduSegManagement.FindInteractTmplCode(16) <> '';
    ...
    begin
        IF InvHdr."Prepayment Credit Memo" THEN
            EXIT(Trl('PurchPrepaymentCreditMemo%1'));
        EXIT(Trl('PurchCreditMemo%1'));
    end;

    local procedure wlFncGetPurchCreditMemoLines(var vRecPurchCrMemoHeader: Record "Purch. Cr. Memo Hdr."; var vRecPurchCrMemoLine: Record "Purch. Cr. Memo Line");
    var
        wlRecPurchCrMemoLine: Record "Purch. Cr. Memo Line";
    begin
        wlRecPurchCrMemoLine.SETRANGE("Document No.", vRecPurchCrMemoHeader."No.");
        IF wlRecPurchCrMemoLine.FINDSET THEN
            REPEAT
                vRecPurchCrMemoLine := wlRecPurchCrMemoLine;
                vRecPurchCrMemoLine.INSERT;
            UNTIL wlRecPurchCrMemoLine.NEXT = 0;
    end;

    local procedure wlFncFormatDocumentFields(pRecPurchCrMemoHeader: Record "Purch. Cr. Memo Hdr.");
    var
        wlPurchPersonText: Text[30];
        wlCurrencyCode: Code[10];
    begin
        // WITH pRecPurchCrMemoHeader DO BEGIN//Krishna
        wlCurrencyCode := pRecPurchCrMemoHeader."Currency Code";
        ...
        wgCduFormatDoc.SetPurchaser(wgRecSalesPurchPerson, pRecPurchCrMemoHeader."Purchaser Code", wlPurchPersonText);
        IF pRecPurchCrMemoHeader."Applies-to Doc. No." = '' THEN
            wgAppliesToText := ''
        ELSE
            wgAppliesToText := STRSUBSTNO(Trl('AppliesTo%1%2'), Trl(FORMAT(pRecPurchCrMemoHeader."Applies-to Doc. Type")), pRecPurchCrMemoHeader."Applies-to Doc. No.");
        //END;
    end;

    local procedure wlFncFormatAddressFields(var vRecPurchCrMemoHeader: Record "Purch. Cr. Memo Hdr.");
    var
        wlRecRespCenter: Record "Responsibility Center";
    begin
        wgCduFormatAddr.GetCompanyAddr(vRecPurchCrMemoHeader."Responsibility Center", wlRecRespCenter, wgRecCompanyInfo, wgCompanyAddr);
        wgCompanyInfoPhoneNo := wgRecCompanyInfo."Phone No.";
        wgCompanyInfoFaxNo := wgRecCompanyInfo."Fax No.";
        wgCduFormatAddr.PurchCrMemoPayTo(wgPayToVendAddr, vRecPurchCrMemoHeader);
        wgCduFormatAddr.PurchCrMemoShipTo(wgShipToAddr, vRecPurchCrMemoHeader);
    end;
}
