report 72105 "R404 Purchase - Quote DCR"
{
    ...
    dataset
    {
        dataitem(PurchHdr; "Purchase Header")
        {
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                column(PurchQuoteTitle; STRSUBSTNO(Trl('PurchQuote%1'), wgCopyText))
                {
                }
            ...
            begin
                ...
                IF wlRecRespCenter.GET("Responsibility Center") THEN BEGIN
                    wgCduFormatAddr.RespCenter(wgCompanyAddr, wlRecRespCenter);
                    wgRecCompanyInfo."Phone No." := wlRecRespCenter."Phone No.";
                    wgRecCompanyInfo."Fax No." := wlRecRespCenter."Fax No.";
                END ELSE BEGIN
                    wgRecCompanyInfo."Phone No." := wgCompanyInfoPhoneNo;
                    wgRecCompanyInfo."Fax No." := wgCompanyInfoFaxNo;
                    wgCduFormatAddr.Company(wgCompanyAddr, wgRecCompanyInfo);
                END;

                IF "Purchaser Code" = '' THEN BEGIN
                    CLEAR(wgRecSalesPurchPerson);
                END ELSE BEGIN
                    wgRecSalesPurchPerson.GET("Purchaser Code");
                END;
                ...
                wgCduFormatAddr.PurchHeaderBuyFrom(wgBuyFromAddr, PurchHdr);

                wgCduFormatAddr.PurchHeaderPayTo(wgPayToVendAddr, PurchHdr);

                wgCduFormatAddr.PurchHeaderShipTo(wgShipToAddr, PurchHdr);
                ...
                IF NOT CurrReport.PREVIEW THEN BEGIN
                    ...
                    IF wgLogInteraction THEN BEGIN
                        ...
                        wgCduSegManagement.LogDocument(
                          11, "No.", "Doc. No. Occurrence", "No. of Archived Versions", DATABASE::Vendor, "Buy-from Vendor No.",
                ...
                ////wgHideLineDiscount := wgCduDocCreatorReportFunctions.wgFncHideLineDiscount(wlRecRef);//Krishna//krihsna
            end;
        }
    }
    ...
    requestpage
    {
        ...
        begin
            ...
            wgArchiveDocument := (wgRecPurchSetup."Archive Quotes" <> wgRecPurchSetup."Archive Quotes"::Never) OR (wgRecPurchSetup."Archive Orders");
            wgLogInteraction := wgCduSegManagement.FindInteractTmplCode(11) <> '';
